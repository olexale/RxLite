...
namespace RxLite
{
    ...
    ///     fail.
    /// </summary>
    internal abstract class ReactiveDerivedCollection<TValue> : ReactiveList<TValue>, IReactiveDerivedList<TValue>,
        IDisposable
    {
    ...
    internal class ReactiveDerivedCollection<TSource, TValue> : ReactiveDerivedCollection<TValue>, IDisposable
    {
        ...
        private readonly List<int> indexToSourceIndexMap;
        private CompositeDisposable inner;
        private readonly List<TSource> sourceCopy;
            ...
            IScheduler scheduler)
        {
            ...
            inner = new CompositeDisposable();
            indexToSourceIndexMap = new List<int>();
            sourceCopy = new List<TSource>();

            inner.Add(Disposable.Create(() =>
            {
        ...
        private void wireUpChangeNotifications()
        {
            ...
            if (signalReset != null)
            {
                inner.Add(signalReset.ObserveOn(scheduler).Subscribe(x => Reset()));
            }
        }
        ...
        private void onItemChanged(TSource changedItem)
        {
            ...
            {
                var currentDestinationIndex = getIndexFromSourceIndex(sourceIndex);
                ...
                else if (isIncluded && shouldBeIncluded)
                {
                    ...
                    var newItem = selector(changedItem);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (ReferenceEquals(newItem, this[currentDestinationIndex]))
                            {
                                var newDestinationIndex = newPositionForExistingItem(
        ...
        private bool canItemStayAtPosition(TValue item, int currentIndex)
        {
            ...
            if (hasPrecedingItem)
            {
                var isGreaterThanOrEqualToPrecedingItem = orderer(item, this[currentIndex - 1]) >= 0;
            ...
            var hasSucceedingItem = currentIndex < Count - 1;

            if (hasSucceedingItem)
            {
                var isLessThanOrEqualToSucceedingItem = orderer(item, this[currentIndex + 1]) <= 0;
        ...
        {
            return indexToSourceIndexMap.IndexOf(sourceIndex);
        }
        ...
        private void onSourceCollectionChanged(NotifyCollectionChangedEventArgs args)
        {
            ...
            if (args.Action == NotifyCollectionChangedAction.Move)
            {
                ...
                var oldSourceIndex = args.OldStartingIndex;
                var newSourceIndex = args.NewStartingIndex;

                sourceCopy.RemoveAt(oldSourceIndex);
                sourceCopy.Insert(newSourceIndex, (TSource) args.NewItems[0]);

                var currentDestinationIndex = getIndexFromSourceIndex(oldSourceIndex);
                ...
                var value = base[currentDestinationIndex];
            ...
            if (args.OldItems != null)
            {
                ...
                {
                    var destinationIndex = getIndexFromSourceIndex(args.OldStartingIndex + i);
                ...
                var removedCount = args.OldItems.Count;
        ...
        {
            for (var i = 0; i < indexToSourceIndexMap.Count; i++)
            {
        ...
        private void shiftSourceIndicesInRange(int rangeStart, int rangeStop, int value)
        {
            for (var i = 0; i < indexToSourceIndexMap.Count; i++)
            {
                var sourceIndex = indexToSourceIndexMap[i];
        ...
        {
            var destinationIndex = positionForNewItem(sourceIndex, value);
        ...
        private int positionForNewItem(int sourceIndex, TValue value)
        {
            ...
            return orderer == null
                ...
                : positionForNewItem(this, 0, Count, value, orderer);
        }
            ...
            IList<T> list, T item, int currentIndex, Func<T, T, int> orderer = null)
        {
            ...
            var comparand = list[precedingIndex >= 0 ? precedingIndex : succeedingIndex];
            ...
            var ix = positionForNewItem(list, min, max - min, item, orderer);
    ...
    public static class ReactiveCollectionMixins
    {
        ...
        /// </param>
        /// <param name="onError">
        ///     The handler for errors from the Observable. If
        ///     not specified, an error will go to DefaultExceptionHandler.
        /// </param>
    ...
    public static class ObservableCollectionMixin
    {
        ...
        ///     An action that is called on each item when
        ///     it is removed.
        /// </param>
        /// <param name="filter">
        ///     A filter to determine whether to exclude items
        ///     in the derived collection.
        /// </param>
        /// <param name="orderer">
        ///     A comparator method to determine the ordering of
        ///     the resulting collection.
        /// </param>
        /// <param name="signalReset">
        ///     When this Observable is signalled,
        ...
        public static IReactiveDerivedList<TNew> CreateDerivedCollection<T, TNew>(
            ...
            IScheduler scheduler = null)
        {
            return This.CreateDerivedCollection(selector, onRemoved, filter, orderer, (IObservable<Unit>) null,
                scheduler);
        }
        ...
        public static IReactiveDerivedList<TNew> CreateDerivedCollection<T, TNew>(
            ...
            IScheduler scheduler = null)
        {
            return This.CreateDerivedCollection(selector, default(Action<TNew>), filter, orderer,
                (IObservable<Unit>) null, scheduler);
        }
    }
}